...
router.connect('/', middleware, async (request, repsonse, next) => {
    await request.text();
    next();
}, async (request, response) => {
    const body = await request.json();
...
router.post('/', {
    max_body_length: 250,
}, middleware, middleware, middleware, async (request, repsonse, next) => {
    await request.text();
    next();
}, async (request, response) => {
    const body = await request.json();
...
router
    ...
    .delete({
    max_body_length: 250,
}, middleware, [middleware, middleware], async (request, response) => {
    const body = await request.json();
});
